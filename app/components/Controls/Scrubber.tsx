...
import { useAudioPlayer, useEventListener, useInterval, useViewContext } from "hooks";
...
const Scrubber = ({ isScrubbing }: Props) => {
  ...
  // Check if the active view is Now Playing or Cover Flow
  ...
  const scrubForward = useCallback(() => {
    // Only process if controls are active AND scrubbing mode is enabled
    if (!isControlsActiveRef.current || !isScrubbing) return;
