...
const useScrollHandler = (
  ...
  // Use ref to track active state to avoid stale closures
  // Update synchronously during render, not in effect
  ...
  // Use refs for options to avoid stale closures in scroll handlers
  // Update synchronously during render, not in effect
  const optionsRef = useRef(options);
  optionsRef.current = options;

  const handleForwardScroll = useCallback(() => {
    // Use ref to get current active state to avoid stale closure
    if (!isActiveRef.current) return;
    
    setIndex((prevIndex) => {
      const currentOptions = optionsRef.current;
      if (prevIndex < currentOptions.length - 1) {
        triggerHaptics(10);
        handleCheckForPreview(prevIndex + 1);

        // Trigger near-end-of-list callback when we're halfway through the current list.
        if (prevIndex === Math.round(currentOptions.length / 2)) {
          onNearEndOfList?.(currentOptions.length);
        }
